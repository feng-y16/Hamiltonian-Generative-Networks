...
class HamiltonianEGNNNet(nn.Module):
    """The Hamiltonian network, composed of 6 convolutional layers and a final linear layer.
    """
    ...
    def __init__(self,
        ...
        if all(var is None for var in (hidden_conv_layers, n_filters,
                                       kernel_sizes, strides, paddings)):
            hidden_layers = HamiltonianEGNNNet.DEFAULT_PARAMS[
                'hidden_conv_layers']
            n_filters = HamiltonianEGNNNet.DEFAULT_PARAMS['n_filters']
            kernel_sizes = HamiltonianEGNNNet.DEFAULT_PARAMS['kernel_sizes']
            strides = HamiltonianEGNNNet.DEFAULT_PARAMS['strides']
            paddings = HamiltonianEGNNNet.DEFAULT_PARAMS['paddings']
        elif all(var is not None for var in (hidden_conv_layers, n_filters,
                                             kernel_sizes, strides, paddings)):
            # If no Nones, check consistency
            assert len(n_filters) == hidden_conv_layers + 1,\
    ...
    def forward(self, q, p):
        ...
        N, C, H, W = x.size()
        C //= 2
        q, p = x[:, : C, :, :].view(N, C, -1), x[:, C: , :, :].view(N, C, -1)
        q, p = self.hidden_layers(p, q)
        hidden = torch.cat((q, p), dim=1).view(N, 2 * C, H, W)
